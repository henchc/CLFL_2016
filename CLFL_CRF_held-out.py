...
#This program trains a CRF tagger to scan MHG poetry
...
def prepcrf(taggedstring):
    ...
    training = []
    for line in tagged:
        news = [nltk.tag.str2tuple(t) for t in line.split()] #make tags into tuples
        if len(news) > 0:
            newtups = []
            for tup in news:
                syllweight = (tup[0], syllableend(tup[0])+syllableweight(tup[0]), tup[1]) #adds syllable characteristics to tuple for being feature
...
def syllable2features(line, i):
    ...
    if i > 0: #preceding syllable
        ...
        features.extend([
            ...
            '(-1:syllable[-1] + syllable[0])=%s' + (syllablen1[-2:] + syllable[:2]),
    ...
    if i < len(line)-1: #following syllable
        ...
        features.extend([
            ...
            '(-1:syllable[-1] + syllable[0])=%s' + (syllable[-2:] + syllable1[:2]),
